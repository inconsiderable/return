...
function ConsiderationMap({
  ...
  const handleNodeCanvasObject = useCallback(
    (node: any, ctx: any, globalScale: any) => {
      const label =
        (node.focaleIndex !== -1 ? `(${node.focaleIndex}) #` : '') +
        (node.label || shortenB64(node.pubkey));
        ...
        <IonCardHeader className="ion-padding-horizontal">
          ...
          >
            <IonSearchbar
              ...
              //searchIcon={returnDownBackOutline}
              clearIcon={returnDownBackOutline}
...
const scaleEdgeWeight = (weight: number, maxWeight: number) => {
  return Math.log(1 + weight) / Math.log(1 + maxWeight);
};
  ...
  return (
    <div className="ion-padding">
      <IonList>
        ...
        <IonItem>
          <IonToggle>Hide time/pass nodes?</IonToggle>
        </IonItem>
        <IonItem>
          <IonToggle>Hide system nodes?</IonToggle>
        </IonItem>
      </IonList>
    </div>
  );
