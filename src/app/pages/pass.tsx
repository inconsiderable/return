...
import { ImportHeart } from '../components/importHeart';

const Pass = () => {
  const { colorScheme, pushConsideration, profile, requestProfile } =
  ...
  const { publicKeys, selectedKey, setSelectedKey, importHeart, deleteHeart } =
    useHeart();

  const keyProfile = profile(selectedKey);
  const pubKeyTracking = keyProfile?.imbalance;
  ...
  return (
    <PageShell
      ...
      renderBody={() => (
        <>
          {!selectedKey ? (
            <ImportHeart importHeart={importHeart} />
          ) : (
            <>
