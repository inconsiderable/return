...
export const signConsideration = async (
  ...
  const heart = await getHeart(passPhrase, Number(heartIndex + 1));

  const keyPair = heart[heartIndex];
...
const importKeys = async (passPhrase: string, returnedKeyCount: number) => {
  ...
  return (await getHeart(passPhrase, returnedKeyCount)).map(
...
export const useHeart = () => {
  ...
  const importHeart = async (passphrase: string) => {
